...
import { ENGINE_MODEL_IDS, FORMAT_MIME, GEMINI_MODEL, IMAGE_STYLE_PROMPTS, } from "./config.js";
export function resolveApiKey() {
    const key = process.env.GOOGLE_API_KEY ?? process.env.GEMINI_API_KEY;
    if (!key) {
        console.error("Error: GOOGLE_API_KEY または GEMINI_API_KEY 環境変数を設定してください");
        process.exit(1);
    }
    return key;
}
...
export async function enhancePrompt(client, theme, style, context = "") {
...
export async function generateImage(client, prompt, options = {}) {
    ...
    if (!response.generatedImages?.length) {
        console.error("Error: 画像が生成されませんでした（安全フィルターの可能性あり）");
        process.exit(1);
    }
